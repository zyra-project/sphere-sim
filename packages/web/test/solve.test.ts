...
test('a solve recovers the rig from photographs and improves the alignment', { timeout: 300_000 }, () => {
  ...
  const req = request();
  const result = runSolve(req, (p) => phases.push(p.phase));
  ...
  // Progress was reported in order, and reached the end.
  assert.deepEqual(phases, ['capture', 'decode', 'initialize', 'score']);
