 ...
 * measures how often the hole is actually reached rather than arguing about it.
 *
...
/**
 * What one photograph says about itself.
 *
 * `litFraction` is the load-bearing number and the mean is not. A white frame
 * lights the whole of one projector's crescent, a Gray plane lights about half
 * of it, and a black frame lights none — so the FRACTION OF THE FRAME ABOVE A
 * MID-LEVEL separates the three, and separates them by a ratio that does not
 * depend on how much of the picture the sphere fills. The mean carries the same
 * ordering and is reported because it is what an operator would look at, but it
 * moves with exposure and with the room in ways the fraction does not.
 */
export interface FrameObservation {
  ...
  mean: number;
  /** Fraction of measured pixels above the frame's own mid-level. */
  litFraction: number;
}
 ...
 * rests on.
 */
export function observe(image: LinearImage, ordinal: number, mask?: Uint8Array): FrameObservation {
  ...
  if (count === 0) return { ordinal, mean: 0, litFraction: 0 };
  const mid = (lo + hi) / 2;
  let lit = 0;
  for (let i = 0; i < n; i++) {
    if (mask !== undefined && mask[i] !== 1) continue;
    const v = image.data[i * stride];
    if (Number.isFinite(v) && v > mid) lit++;
  }
  return { ordinal, mean: sum / count, litFraction: lit / count };
}
 ...
 * still an order of magnitude above the noise on a frame-wide pixel count. It is
 * a threshold on a quantity this project has never measured on a real sphere, so
 * it is stated as a constant with its reasoning rather than buried, and
 * `docs/EXPERIMENT-8.md` measures what it actually costs.
 */
  ...
  if (margin < MIN_CLASSIFY_MARGIN) {
    problems.push(
      `The white and black frames cannot be reliably told from the patterned ones: the closest ` +
        `frame sits ${margin.toFixed(3)} from a threshold, under the ${MIN_CLASSIFY_MARGIN} this ` +
        `needs. That usually means the sphere fills too little of the frame, the room is bright ` +
        `enough to lift the black frames, or the exposure is clipping the white ones. Nothing ` +
        `below this line would be a measurement; it would be a segmentation of noise.`,
    );
    return refuse();
  }
