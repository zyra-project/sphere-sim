...
| [`docs/ARBITRARY-SHAPES.md`](docs/ARBITRARY-SHAPES.md) | **Feasibility study, and the running record of building it.** What it takes to render an uploaded GLB with projectors anywhere, and where the sphere is load-bearing. Phases 0, 1 and 4 are implemented, and 3 all but the polar mask: a `.glb` dropped on the page is traced, lit, blended, lit by any number of projectors placed anywhere, and exportable as a warp mesh. Phase 2's traversal is proven against the simulator; the live view is still the sphere |
